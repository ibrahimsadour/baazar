...
class Country extends BaseModel
{
	...
	{
		// Check if the 'currencies' column is available in the Country model
		$cacheId = 'currenciesColumnIsAvailableCountryTable';
		$cacheExpiration = (int)config('settings.optimization.cache_expiration', 86400) * 5;
		$currenciesColumnIsAvailable = cache()->remember($cacheId, $cacheExpiration, function () {
			return Schema::hasColumn($this->getTable(), 'currencies');
		});
		
		if ($currenciesColumnIsAvailable) {
