...
function getAuthField($entity = null): string
{
	...
	$authField = (in_array($authField, $validAuthFields)) ? $authField : $defaultAuthField;
...
function getAuthFieldOnTokenPage(): ?string
{
	...
	if (request()->segment(2) == 'verify') {
		if (
			...
			&& in_array(request()->segment(3), ['email', 'phone'])
		) {
...
function isEnabledField(?string $field): bool
{
	...
	// Front Register Form
	if ($field == 'phone') {
		$isEnabled = (config('settings.sms.enable_phone_as_auth_field') == '1');
	}
	if (request()->filled('auth_field')) {
		$isEnabled = ($field == request()->input('auth_field'));
	} else {
		$guard = isFromApi() ? 'sanctum' : null;
		if (auth($guard)->check()) {
			$user = auth($guard)->user();
			$isEnabled = (isset($user->auth_field) && $field == $user->auth_field);
		}
	}
	if ($field == 'username') {
